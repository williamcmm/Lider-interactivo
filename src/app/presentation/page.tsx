...
function SearchParamsComponent() {
  ...
  useEffect(() => {
    ...
    if (lessonId) {
      getLessonById(lessonId).then((res) => {
        if (!res.ok) return;
        const l = res.lesson as DbLessonWithContainer;
  ...
  }, [searchParams, setCurrentFragmentId]);
          ...
          {currentFragment.slides && currentFragment.slides.length > 0 ? (
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: currentFragment.slides[0].content,
              }}
            />
          ) : (
...
export default function PresentationPage() {
  return (
    <Suspense fallback={<Loading message="Preparando presentación..." />}>
