...
import { FiPlus, FiEdit, FiTrash2, FiBook, FiList, FiSave, FiArrowLeft, FiX, FiChevronDown, FiChevronUp, FiEye } from 'react-icons/fi';
...
export function AdminPanel({}: AdminPanelProps) {
  ...
  const [selectedContainer, setSelectedContainer] = useState<StudyContainer | null>(null);
  ...
  const mockSeminars: Seminar[] = [
    {
      ...
      description: 'Principios fundamentales del liderazgo cristiano',
      lessons: [
        {
          id: 'lesson-1',
          title: 'Fundamentos del Liderazgo',
          content: '<p>Contenido sobre fundamentos</p>',
          containerId: 'sem-1',
...
// ...existing code...
  ...
  const handleLessonsCountChange = (count: number) => {
    const newCount = Math.max(1, Math.min(50, count)); // Entre 1 y 50 lecciones
    setCreationForm(prev => ({
      ...prev,
      lessonsCount: newCount,
      lessons: Array.from({ length: newCount }, (_, i) => {
        // Mantener títulos existentes si los hay
  ...
  // Manejar cambios en archivos de audio del seminario
  ...
  // Agregar nuevo archivo de audio
  const addAudioFile = () => {
  ...
  // Manejar fragmentos de lecciones
  const addFragment = () => {
  ...
  // Funciones para manejar audio de narración en fragmentos
  ...
  const saveIndividualFragment = (index: number) => {
    ...
    // Actualizar la lección con los fragmentos actuales
    ...
    if (editingContainer.type === 'seminar') {
      setSeminars(seminars.map(s => 
        s.id === editingContainer.id ? updatedContainer as Seminar : s
      ));
    } else {
      setSeries(series.map(s => 
        s.id === editingContainer.id ? updatedContainer as Series : s
      ));
    }
  ...
  const handleSaveContainer = () => {
    ...
    if (activeTab === 'seminars') {
      setSeminars([...seminars, newContainer as Seminar]);
    } else {
      setSeries([...series, newContainer as Series]);
    }

    // Después de guardar, pasar a la edición de lecciones
    setEditingContainer(newContainer);
    setIsEditingLessons(true);
    setSelectedLessonIndex(0);
    setIsCreatingContainer(false);
    
    // Inicializar fragmentos de la primera lección
    const firstLesson = newContainer.lessons[0];
    if (firstLesson) {
  ...
  const handleEditLessons = (container: Seminar | Series) => {
    ...
    // Cargar fragmentos de la primera lección si existe
  ...
  const handleSelectLesson = (index: number) => {
    ...
    if (lesson && lesson.fragments) {
      ...
      setSelectedFragmentIndex(0);
      setEditingFragmentIndex(null); // Resetear edición al cambiar de lección
    } else {
      setFragmentsData([]);
    }
  };

  const handleSaveFragments = () => {
    if (!editingContainer) return;
    ...
    setEditingContainer(updatedContainer);
    alert('Fragmentos de la lección guardados exitosamente');
  };
  ...
  const handleEditContainer = (container: Seminar | Series) => {
    // Convertir a StudyContainer para unificar el manejo
    const studyContainer: StudyContainer = {
      ...container,
      type: activeTab === 'seminars' ? 'seminar' : 'series'
    };
    setSelectedContainer(studyContainer);
  ...
  const handleCreateLesson = (containerId: string) => {
    const container = currentData.find(c => c.id === containerId);
    if (container) {
      const studyContainer: StudyContainer = {
        ...container,
        type: activeTab === 'seminars' ? 'seminar' : 'series'
          ...
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                    ...
                    <p className="text-gray-600">Gestiona seminarios, series y lecciones</p>
                  </h1>
                  </div>
                  {!isCreatingContainer && !isEditingLessons && (
                    <button onClick={handleCreateContainer} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              ...
              {isCreatingContainer ? (
                <div className="bg-white rounded-lg shadow-sm p-6">
                  ...
                  <div className="flex justify-between items-center mb-6 pb-4 border-b">
                    <h2 className="text-xl font-bold text-gray-900">Crear {activeTab === 'seminars' ? 'Seminario' : 'Serie'}</h2>
                    <button onClick={handleCancelCreation} className="inline-flex items-center px-3 py-1.5 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors">
                      <FiArrowLeft className="mr-2" />
                      Volver
                    </button>
                  </div>
                  {/* Formulario principal y resto del contenido */}
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Ingresa el título del seminario o serie"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                      ...
                      <textarea
                        ...
                        className="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                        rows={3}
                        placeholder="Descripción breve del seminario o serie"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleLessonsCountChange(Number(e.target.value))}
                        className="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        min={1}
                        max={50}
                      />
                    ...
                    {/* Formulario de lecciones (repetido según el contador) */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                        Lecciones
                      </label>
                      ...
                      <div className="space-y-4">
                        {creationForm.audioFiles.map((audioFile, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            ...
                            {creationForm.audioFiles.length > 1 && (
                              <button
                ...
                <div className="bg-white rounded-lg shadow-sm max-h-[calc(100vh-120px)] overflow-y-auto">
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center mb-6 pb-4 border-b sticky top-0 bg-white z-10">
                      <div>
                        <h2 className="text-xl font-bold text-gray-900">Editando: {editingContainer.title}</h2>
                        <p className="text-gray-600">Configura el contenido de cada lección</p>
                      </div>
                      <button onClick={handleFinishEditingLessons} className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                        <FiSave className="mr-2" />
                        Finalizar Edición
                      </button>
                    </div>
                    {/* Resto de la edición de lecciones y fragmentos */}
                    {/* ...existing code... */}
                  </div>
                </div>
              ) : (
            ...
            <div className="space-y-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-blue-700 text-sm">
                  Cada lección está dividida en fragmentos. Cada fragmento contiene: material de lectura, diapositiva y ayudas de estudio.
                </p>
              </div>
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-md p-3 space-y-2">
                                  <div className="flex gap-2">
                                    <input
                                      ...
                                      placeholder="Nombre del audio de narración"
                                    />
                                    <select
                                  ...
                                  {fragment.narrationAudio.type === 'local' && (
                                    <div className="text-sm text-gray-600">
                                      <p>Archivo local: {fragment.narrationAudio.name || 'Sin especificar'}</p>
                                      <p className="text-xs text-gray-500">Los archivos locales se cargarán desde el dispositivo durante la reproducción</p>
                                    </div>
                                  )}
                                </div>
                              )}
                  ...
                  <ul className="space-y-1">
                    {editingContainer.audioFiles.map((file, index) => (
                      <li key={index} className="text-sm text-green-700">
                        • MP3 {index + 1}: {file.name || 'Sin nombre'} ({file.type})
                      </li>
                    ))}
                  </ul>
                ) : (
              ...
              <div className="flex justify-center pt-6 border-t">
                ...
                >
                  <FiSave className="mr-2" />
                  Guardar Fragmentos de esta Lección
                </button>
              </div>
            </div>
          </div>
            ...
            {currentData.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-6">
                  Comienza creando tu primer {activeTab === 'seminars' ? 'seminario' : 'serie'}
                </p>
                <button
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center mt-2 text-sm text-gray-500">
                          ...
                          <span>Orden: {container.order}</span>
                        </div>
                      </div>
                      <div className="flex space-x-2 ml-4">
                        <button
                          ...
                          title="Editar contenido de lecciones"
                        >
                          <FiEdit />
                        </button>
                        <button
                          onClick={() => handleCreateLesson(container.id)}
